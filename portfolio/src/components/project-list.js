      ...
      <div id="main">
        <section id="one" className="tiles">
          {projectData.projects.map(project => (
            <article key={project.id} style={{backgroundImage: "url('" + project.img +"')"}}>
              <span className="image">
                <img src={project.img} alt={project.title} />
              </span>
              <header className="major">
                <h3><a href="#" onClick={event => props.handleRedirection(event, project.name)} className="link">{project.title}</a></h3>
              </header>
            </article>
          ))}
          </section>
